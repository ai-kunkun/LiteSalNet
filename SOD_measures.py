...
def mask_normalize(mask):
    # input 'mask': HxW
    # output: HxW [0,255]
    return mask / (np.amax(mask) + 1e-8)  # np.amax() 


def compute_mae(mask1, mask2):
    # input 'mask1': HxWxn  (asumme that all the n channels are the same and only the first channel will be used)
    #       'mask2': HxWxn
...
def compute_ave_MAE_of_methods(gt_name_list, rs_dir_lists):
    ...
    for i in range(0, num_gt):
        # print('-Processed %d/%d' % (i + 1, num_gt), end='\r')  # end='\r'表示每一次的打印都会回到本行的开头
...
def compute_pre_rec(gt, mask, mybins=np.arange(0, 256)):
    ...
    pp_hist, pp_edges = np.histogram(pp,
                                     bins=mybins)  # count pixel numbers with values in each interval [0,1),[1,2),...,[mybins[i],mybins[i+1]),...,[254,255)
...
def compute_PRE_REC_FM_of_methods(gt_name_list, rs_dir_lists, beta=0.3):
    ...
    for i in range(0, num_gt):
        ...
        for j in range(0, num_rs_dir):
            pre, rec, f = np.zeros(len(mybins)), np.zeros(len(mybins)), np.zeros(
                len(mybins))  # pre, rec, f or one mask w.r.t different thresholds
            try:
...
def plot_save_pr_curves(PRE, REC, method_names, lineSylClr, linewidth, xrange=(0.0, 1.0), yrange=(0.0, 1.0),
    ...
    ## draw dataset name
    dataset_name_new = None
    plt.text((xrange[0] + xrange[1]) / 2.0, yrange[0] + 0.02, dataset_name_new, horizontalalignment='center', fontsize=20,
